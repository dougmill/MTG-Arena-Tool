...
//
function selectAdd(div, callback, width = null) {
  div.each(function() {
    var $this = $(this),
      numberOfOptions = $(this).children("option").length;

    $this.addClass("select-hidden");
    let select = $('<div class="select"></div>');
    if (width) select.width(width);
    $this.wrap(select);
      ...
      if (renderer == 0) {
        ...
        for (let i = 0; i < 4; i++) {
          if (cardsNew[_card.id] != undefined && i < cardsNew[_card.id]) {
            $('<div class="inventory_card_quantity_orange"></div>').appendTo(
              hoverCardQuantity
            );
          } else if (i < cards[_card.id]) {
            $('<div class="inventory_card_quantity_green"></div>').appendTo(
              hoverCardQuantity
            );
          } else {
            $('<div class="inventory_card_quantity_gray"></div>').appendTo(
              hoverCardQuantity
            );
          }
        }
